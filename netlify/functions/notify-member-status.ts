...
export const handler = async (event: { httpMethod: string; headers: Record<string, string | undefined>; body: string | null }) => {
  ...
  if (!brevoApiKey || !senderEmail) return { statusCode: 503, body: JSON.stringify({ message: "Brevo sender configuration অসম্পূর্ণ" }) };
  ...
  if (memberError) return { statusCode: 500, body: JSON.stringify({ message: "Admin role যাচাই করা যায়নি" }) };
  if (!member || member.role !== "admin" || member.status !== "approved") return { statusCode: 403, body: JSON.stringify({ message: "শুধু অনুমোদিত Admin এই notification পাঠাতে পারবেন" }) };
  ...
  if (!input.email || !input.fullName || !input.status) return { statusCode: 400, body: JSON.stringify({ message: "সদস্যের email, নাম ও status প্রয়োজন" }) };

  const approved = input.status === "approved";
  const subject = approved ? "সমিতি-নাইন্টি ত্রি: সদস্য অনুমোদন সম্পন্ন" : "সমিতি-নাইন্টি ত্রি: সদস্যপদ স্থগিত";
  const message = approved ? "আপনার সদস্য আবেদন Admin অনুমোদন করেছেন। এখন email ও password দিয়ে নিরাপদ হিসাব ব্যবস্থাপনায় প্রবেশ করতে পারবেন।" : "আপনার সদস্যপদ বর্তমানে স্থগিত করা হয়েছে। বিস্তারিত জানতে Admin-এর সঙ্গে যোগাযোগ করুন।";
  ...
  if (!response.ok) return { statusCode: 502, body: JSON.stringify({ message: "Brevo email পাঠাতে পারেনি" }) };
