    ...
    <DialogContent className="member-auth-dialog max-w-md max-h-[90vh] overflow-y-auto bg-white text-left" dir="ltr">
      ...
      <form className="grid gap-4" onSubmit={submit}>
        {mode === "signup" && <>
          ...
          <div className="flex items-center gap-3 rounded-lg border border-dashed border-emerald-200 bg-emerald-50 p-3"><div className="grid h-12 w-12 place-items-center overflow-hidden rounded-full bg-white text-emerald-700">{form.photoUrl ? <img src={form.photoUrl} alt="সদস্যের ছবি" className="h-full w-full object-cover" /> : <ImagePlus className="h-5 w-5" />}</div><div className="min-w-0 flex-1"><strong className="block text-xs">প্রোফাইল ছবি</strong><span className="block text-[10px] text-slate-500">ছবি সংকুচিত হয়ে নিরাপদে সংরক্ষিত হবে</span></div><label className="cursor-pointer rounded-md bg-white px-2 py-1 text-xs font-bold text-emerald-700">ছবি দিন<input className="hidden" type="file" accept="image/*" onChange={onPhoto} /></label></div>
        </>}
