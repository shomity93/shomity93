...
export default function MemberTransactionForm({ members, initial, onSubmit, onCancel }: Props) {
  ...
  const [values, setValues] = useState({
    member_id: initial?.member_id ?? initialMember?.id ?? "",
    ...
    attachment_type: initial?.attachment_type ?? "",
    attachment_size: initial?.attachment_size ? String(initial.attachment_size) : "",
  });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!values.member_id) return setError("সদস্য নির্বাচন করুন");
    if (!values.description.trim()) return setError("বিবরণ লিখুন");
    ...
    try {
      let attachment = {
        ...
        attachment_size: values.attachment_size ? Number(values.attachment_size) : null,
      };
      if (pendingFile) {
        const uploaded = await uploadCooperativeFile(pendingFile, "receipts");
        attachment = {
          attachment_url: uploaded.url,
          attachment_name: uploaded.compressed.name,
          attachment_type: uploaded.compressed.type || "ফাইল",
          attachment_size: Math.max(1, Math.round(uploaded.compressed.size / 1024)),
        };
      }
      await onSubmit({
        id: initial?.id,
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && !busy && onCancel()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto bg-white text-slate-900">
        <DialogHeader><DialogTitle>{initial ? "সদস্যের হিসাব সম্পাদনা" : "সদস্যের নতুন হিসাব"}</DialogTitle></DialogHeader>
        <form className="grid gap-4" onSubmit={submit}>
          ...
          <div className="flex gap-2"><Button type="submit" disabled={busy}>{busy ? "সংরক্ষণ হচ্ছে…" : "সংরক্ষণ করুন"}</Button><Button type="button" variant="outline" disabled={busy} onClick={onCancel}>বাতিল</Button></div>
        </form>
      </DialogContent>
    </Dialog>
  );
