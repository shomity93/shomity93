...
export default function LedgerForm({ type, members, initial, onSubmit, onCancel }: Props) {
  ...
  const onFileSelected = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? `ফাইল আপলোড হয়নি: ${err.message}` : "ফাইল আপলোড হয়নি; আবার চেষ্টা করুন");
    } finally {
      setBusy(false);
    }
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError("");
    try {
      const file = { url: values.file_url || null, name: values.file_name || null, type: values.file_type || null, size: values.file_size ? Number.parseInt(values.file_size, 10) || null : null };
