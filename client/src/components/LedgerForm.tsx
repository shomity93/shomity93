...
export default function LedgerForm({ type, members, initial, onSubmit, onCancel }: Props) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!values.file_url && !pendingFile) return setError(type === "deposit" ? "রসিদ নির্বাচন করা বাধ্যতামূলক" : "বিল/ভাউচার ফাইল নির্বাচন করা বাধ্যতামূলক");
    setBusy(true);
    setError("");
    try {
      let uploaded = values.file_url ? { url: values.file_url, compressed: { name: values.file_name, type: values.file_type, size: Number.parseInt(values.file_size, 10) || 0 } } : null;
      if (pendingFile) uploaded = await uploadCooperativeFile(pendingFile, type === "deposit" ? "receipts" : "vouchers");
      if (!uploaded?.url) throw new Error(type === "deposit" ? "রসিদ আপলোড করা যায়নি" : "বিল/ভাউচার ফাইল আপলোড করা যায়নি");
      const file = { url: uploaded.url, name: uploaded.compressed.name || values.file_name || null, type: uploaded.compressed.type || values.file_type || null, size: uploaded.compressed.size ? Math.max(1, Math.round(uploaded.compressed.size / 1024)) : (Number.parseInt(values.file_size, 10) || null) };
