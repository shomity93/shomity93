...
import { listApprovedMembers, subscribeToLedgerChanges } from "@/lib/supabase";
...
export default function Home() {
  ...
  const [members, setMembers] = useState<Array<{ member_id: string; full_name: string; photo_url?: string | null }>>([]);
  ...
  const visibleMembers = members.length ? members : ["মোঃ রাকিব হাসান", "ফারজানা আক্তার", "শামীম আহমেদ", "মোছাঃ নুসরাত", "আবু বকর সিদ্দিক", "সাবিনা ইয়াসমিন"].map((full_name, index) => ({ full_name, member_id: `S-00${index + 1}`, photo_url: null }));
  ...
  return <div className="site-shell" dir="ltr"><header className="public-nav"><div className="container nav-inner"><a className="brand" href="#home" onClick={() => setActive("home")}><span className="brand-mark"><Gem className="h-5 w-5" /></span><span><strong>{siteName}</strong><small>সমবায়ে সমৃদ্ধি</small></span></a><nav className={`desktop-links ${mobileNav ? "is-open" : ""}`}>{navItems.map((item) => <a key={item.id} className={active === item.id ? "active" : ""} href={item.id === "hisab" ? "/hisab" : `#${item.id}`} onClick={() => { setActive(item.id); setMobileNav(false); }}>{item.label}</a>)}</nav><div className="nav-actions"><Badge className="live-badge"><span />লাইভ আপডেট{liveTick > 0 ? ` · ${liveTick}` : ""}</Badge>{isAuthenticated ? <span className="user-chip">{userLabel}</span> : <MemberAuthDialog />}<Button variant="ghost" size="icon" className="mobile-menu" onClick={() => setMobileNav(!mobileNav)}>{mobileNav ? <X /> : <Menu />}</Button></div></div></header>
    ...
    <section className="section access-teaser"><div className="container"><div className="access-teaser-card"><div><p className="eyebrow">নিরাপদ সদস্য অঞ্চল</p><h2>হিসাব ব্যবস্থাপনা আলাদা ও সুরক্ষিত</h2><p>সমিতির জমা, খরচ, সদস্যভিত্তিক হিসাব ও রিপোর্ট দেখতে অনুমোদিত সদস্য হিসেবে প্রবেশ করুন। Public homepage-এ কোনো আর্থিক তথ্য প্রকাশ করা হয় না।</p></div><Button className="cta-primary" onClick={() => { window.location.href = "/hisab"; }}>সুরক্ষিত অংশে প্রবেশ <ArrowRight className="ml-2 h-4 w-4" /></Button></div></div></section>
    ...
    <section id="members" className="section members-section"><div className="container"><div className="section-heading"><div><p className="eyebrow">আমাদের পরিবার</p><h2>সদস্যদের সম্মিলিত শক্তি</h2></div><Badge variant="outline">সর্বোচ্চ ৫০ সদস্য</Badge></div><div className="member-row">{visibleMembers.map((member, index) => <div className="member-card" key={member.member_id}><div className={`member-avatar m-${index % 6}`}>{member.photo_url ? <img src={member.photo_url} alt={`${member.full_name} এর ছবি`} className="h-full w-full rounded-full object-cover" /> : member.full_name.slice(0, 2)}</div><strong>{member.full_name}</strong><span>সদস্য · {member.member_id}</span></div>)}</div></div></section>
    </main>

    {cmsOpen && <div className="cms-overlay" onClick={() => setCmsOpen(false)}><Card className="cms-panel" onClick={(e) => e.stopPropagation()}><div className="cms-head"><div><p className="eyebrow">অ্যাডমিন CMS</p><h2>সাইট কনটেন্ট সম্পাদনা</h2></div><Button variant="ghost" size="icon" onClick={() => setCmsOpen(false)}><X /></Button></div><div className="cms-form"><AdminApprovalPanel /><div><Label>সমিতির নাম</Label><Input value={siteName} onChange={(e) => setSiteName(e.target.value)} /></div><div><Label>প্রধান ট্যাগলাইন</Label><Input value={tagline} onChange={(e) => setTagline(e.target.value)} /></div><div><Label>দ্বিতীয় ট্যাগলাইন</Label><Input value={tagline2} onChange={(e) => setTagline2(e.target.value)} /></div><div><Label>যোগাযোগের ইমেইল</Label><Input value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} /></div><div><Label>সংক্ষিপ্ত পরিচিতি</Label><Textarea defaultValue="সঞ্চয়, সহযোগিতা ও স্বচ্ছতার মাধ্যমে একটি নিরাপদ সম্মিলিত ভবিষ্যৎ গড়ার অঙ্গীকার।" /></div><div className="upload-box"><ImagePlus className="h-5 w-5" /><div><strong>গ্যালারি ছবি যোগ করুন</strong><p>ছবি স্বয়ংক্রিয়ভাবে সংকুচিত হয়ে সংরক্ষিত হবে</p></div><label className="upload-button">ফাইল নির্বাচন<input type="file" accept="image/*" onChange={onGalleryUpload} /></label></div><Button onClick={saveCms} className="w-full">{saved ? <><Check className="mr-2 h-4 w-4" />সংরক্ষিত হয়েছে</> : "পরিবর্তন সংরক্ষণ করুন"}</Button></div></Card></div>}
