...
export function calculateDashboardTotals(deposits: Array<number | string>, expenses: Array<number | string>, memberTransactions: MemberTransactionAmount[]) {
  // Deposits are the canonical income ledger. Member-sheet rows contribute only
  // adjustments that are not already represented in the deposits/expenses tables.
  const memberFines = memberTransactions.filter((row) => row.transaction_type === "fine").reduce<number>((sum, row) => sum + Number(row.amount), 0);
  ...
  const totalDeposits = deposits.reduce<number>((sum, amount) => sum + Number(amount), 0) + memberFines;
